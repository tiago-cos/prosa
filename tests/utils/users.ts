...
export async function loginUser(username: string, password: string) {
  ...
  const response = await request(SERVER_URL).post(`/auth/login`).send(payload);
...
export async function logoutUser(refresh_token: string) {
  ...
  const response = await request(SERVER_URL).post(`/auth/logout`).send(payload);
...
export async function refreshToken(refresh_token: string) {
  ...
  const response = await request(SERVER_URL).post(`/auth/refresh`).send(payload);
...
export async function getApiKey(user_id: string, keyId: string, auth?: { jwt?: string; apiKey?: string }) {
  let req = request(SERVER_URL).get(`/users/${user_id}/keys/${keyId}`);
...
export async function getApiKeys(user_id: string, auth?: { jwt?: string; apiKey?: string }) {
  let req = request(SERVER_URL).get(`/users/${user_id}/keys`);
...
export async function deleteApiKey(user_id: string, keyId: string, auth?: { jwt?: string; apiKey?: string }) {
  let req = request(SERVER_URL).delete(`/users/${user_id}/keys/${keyId}`);
...
export async function getPreferences(user_id: string, auth?: { jwt?: string; apiKey?: string }) {
  let req = request(SERVER_URL).get(`/users/${user_id}/preferences`);
